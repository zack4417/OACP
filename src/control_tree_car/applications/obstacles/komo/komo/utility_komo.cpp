...
std::vector<double> data_value(const WorldL & Gs, const Objective*  objectives)
{
    std::vector<double> values;
    //o=objectives;
        const auto & s = objectives->map->scale.d0 ? objectives->map->scale(0) : 1.0;
        const auto & v = (*objectives->map)(Gs);
        const auto & t = objectives->map->target;
        for(auto i=0; i < v.y.d0; ++i)
        {
            values.push_back(v.y(i));
        }
    return values;
}
...
void shift_komos(std::vector<std::shared_ptr<KOMO>>& komos, const OdometryState & o, uint steps)
{
    ...
    // std::cout<<"projection index: "<<index<<std::endl;
    // if(index<2)
    // {index=2;}
    // index=-1;
    ...
    if(index >= 1) // is komo order
    {
        ...
        // start.v = o.v;
        // start.omega = o.omega;
        // std::cout<<"planning start "<<start.x<<" "<<start.y<<std::endl;
        ...
        for(auto&komo: komos)
        {
            ...
            if(vx_end > 0.0)
            {
                ...
                // slide_trajectory(index-1, mu, steps, traj,o.x-proj.x, o.y-proj.y, o.yaw-proj.yaw);
                // first_guess_trajectory(o, steps, traj);
                first_guess_trajectory(o, 0.0, steps, traj);
            ...
            {   // unstuck trajectory
                ...
                first_guess_trajectory(o, 5.0, steps, traj);
            }
            // update_komo(traj, o,komo);
...
void slide_trajectory(uint shift, double mu, uint steps, std::vector<Pose2D>& trajectory,double x_offset, double y_offset,double yaw_offset)
{
    //std::cout << "slide_trajectory smooth.." << shift << " " << mu << std::endl;
    ...
    for(auto i = 0; i < trajectory.size() - shift - 1; ++i)
    {
        // Pose2D trajectory_i, trajectory_i_1;
        // trajectory_i=trajectory[i+shift];
        // trajectory_i_1=trajectory[i+shift+1];
        trajectory[i] = interpolate(trajectory[i+shift], trajectory[i+shift+1], mu);
        // trajectory[i] = interpolate(trajectory_i, trajectory_i_1, mu);
        trajectory[i].y+=y_offset;
        trajectory[i].x+=x_offset;
        // trajectory[i].yaw+=yaw_offset;
    }

    for(auto i = 0; i <= shift; ++i)
    {
        const auto I = trajectory.size() - shift + i - 1;
        trajectory[I].x = last.x + (i + mu) * dx+x_offset;
        trajectory[I].y = last.y + (i + mu) * dy+y_offset;
        // trajectory[I].yaw = yaw+yaw_offset;
        trajectory[I].yaw = yaw;
    }
    return;
}
...
void update_komo(const std::vector<Pose2D>& trajectory, const std::shared_ptr<KOMO>& komo)
{
    ...
    komo->world.q(2) = trajectory[2].yaw;
    // 
    // std::cout<<"q0: "<<komo->world.q(0)<<" q1: "<<komo->world.q(1)<<" q2: "<<komo->world.q(2)<<std::endl;
    ...
    for(auto i=0; i < komo->configurations.d0; ++i)
    {
        ...
        // std::cout<<"i "<<i<<"q0: "<<komo->configurations(i)->q(0)<<" q1: "<<komo->configurations(i)->q(1)<<" q2: "<<komo->configurations(i)->q(2)<<std::endl;
...
void update_komo(const std::vector<Pose2D>& trajectory, const OdometryState & o,const std::shared_ptr<KOMO>& komo)
{
    CHECK_EQ(trajectory.size(), komo->configurations.d0, "traj and komo should have the same size");
