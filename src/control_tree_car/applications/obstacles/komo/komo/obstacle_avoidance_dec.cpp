    ...
    , options_(PARALLEL, true, NOOPT, false)
{
    ...
    for(auto i = 0; i < n_branches_; ++i)
    {
      ...
      objectives.goal_->vars = vars_branch_order_0_;
      objectives.vel_->vars = vars_branch_order_1_;
      objectives.angvel_->vars = vars_branch_order_0_;
      objectives.vel2_->vars = vars_branch_order_1_;
...
void ObstacleAvoidanceDec::obstacle_callback(const visualization_msgs::MarkerArray::ConstPtr& msg)
{
    ...
    for(auto i = 0; i < msg->markers.size() / 2; ++i)
    {
      if(i>=n_obstacles_)
      {
        // ROS_WARN("Received more obstacles than expected (%d), ignoring the rest.", n_obstacles_);
        // break;
      }
      ...
      /// position and geometr
      // std::cout<<"adding"<<j<<"th obstacle"<<std::endl;
      obstacles_[j].position = {m.pose.position.x , m.pose.position.y, 0};
      obstacles_[j].p = 1;
      obstacles_[j].radius = m.scale.y/2;
      if(m.color.a<0.4)
      {
      //   // j++
        obstacles_[j].p = 0.5;
     ...
     if(j<n_obstacles_)
      {
        for(; j < n_obstacles_-1; ++j)
        {
          obstacles_[j].position = {-100, 0, 0};
          obstacles_[j].p = 0;
          obstacles_[j].radius = 0.1;
          std::cout<<"adding extra"<<std::endl;
        }
      }
}
